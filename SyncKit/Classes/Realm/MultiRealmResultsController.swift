                ...
                if let error = error {
                    ...
                    self.listeners.forEach { (observer) in
                        observer.reference?.block(.update(realmCount: self.provider.realms.count,
                                                          deletions: change.deletions(inSection: UInt(index)),
                                                          insertions: change.insertions(inSection: UInt(index)),
                                                          modifications: change.modifications(inSection: UInt(index))))
                    }
                }
            }
        }
