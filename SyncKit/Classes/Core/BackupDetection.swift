...
class BackupDetection: NSObject {
    ...
    fileprivate static let backupDetectionStoreKey = "BackupDetectionStoreKey"
    fileprivate static var applicationDocumentsDirectory: String {
    #if os(iOS) || os(watchOS)
        return NSSearchPathForDirectoriesInDomains(.libraryDirectory, .userDomainMask, true).first!
    #else
        let urls = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
        return urls.last?.appendingPathComponent("com.mentrena.CloudKitSynchronizer").path ?? ""
    #endif
    }
